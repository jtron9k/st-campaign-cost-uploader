...
**Status (2026-08-13):** build complete, reviewed, and verified end to end against a live production tenant. Nothing is queued. Remaining hardening: a large-sheet run, and a duplicate-create question the UI can no longer reach.
...
- Live read-only probe: `uv run python scripts/st_probe.py tenants`, or `... campaigns <tenant> --match google`. It calls only GET methods and never imports `create_cost` or `update_cost`; `tests/test_probe_is_read_only.py` enforces that.
...
- `aliases.py`: per-tenant map from spreadsheet name to campaign ID, persisted in `aliases/<tenant>.json` (gitignored: it holds a real tenant's campaign names, so share it with your team out of band)
...
Confirmed by querying one live tenant, first through the `servicetitan-local` MCP server and later through `scripts/st_probe.py`. They drive most design decisions, so verify before contradicting them, and record anything newly verified here with the date so later sessions do not re-derive it.
...
- Some campaigns carry a record for **every** month over several years, most at `0.0`. Others have **zero** cost records. Which case a campaign is in decides whether a write is an UPDATE or a CREATE, and only a lookup can tell.
- Supervised creates on 2026-08-13 minted records against a campaign that had none, and those records were then set back to `0.0`. **They cannot be removed.** Neither the Marketing v2 API nor `client.py` exposes a delete for cost records, so every CREATE is permanent.
- **`POST /costs`** (verified 2026-08-13) takes the full body `{campaignId, year, month, dailyCost}` and mints a real record with a new id. **Its response body carries no id.** `logs/writes.jsonl` logged `"cost_id": null` while the read-back showed the newly created record with a real id. To learn a created record's id, re-read the campaign's costs and match on `(year, month)`; never build a follow-up request from `create_cost`'s return value.
- **`PATCH /costs/{id}`** (verified 2026-08-12 on a live record, restored to `0.0` afterward) accepts a partial body of just `{"dailyCost": <number>}`, returns HTTP 200 with `{"id": ...}` only, and updates in place with the same id and unchanged `year`, `month`, and `campaignId`. `{"dailyCost": 1.23}` read back as `1.23` and `{"dailyCost": 0}` as `0.0`. Send only `dailyCost` on update; resending the identifying fields is untested and carries risk for no benefit.
- Record ids arrive in contiguous blocks, one per year, which suggests ServiceTitan bulk-creates a year of rows at a time. Treat that as an observation, not a guarantee.
...
Spreadsheets carry campaign **names** and the API needs campaign **IDs**, so name resolution is a core problem. Names are neither unique nor stable, and many carry category strings like `Search||Google||NoCost`. Measured on one live tenant on 2026-08-13: **roughly 2,500 campaigns, and about 1 name in 130 shared by more than one campaign after normalization.** The resolver's refusal to auto-resolve a name matching two campaigns is load-bearing, and the operator resolves ambiguous and unmatched names as a normal case.

### Tenants

`.env.example` shows the shape with four placeholder slugs (`acme_east`, `acme_west`, `northwind`, `globex`). Replace them with your own; any number works. Treat multi-tenant as a requirement.
...
- Every tenant is **production with real business data**. Never issue a POST, PATCH, or PUT outside the app's preview gate, including through the `servicetitan_api_call` escape hatch, without explicit per-instance approval from the operator. Reads are fine.
